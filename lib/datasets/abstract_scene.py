...
#from lib.SceneGraphParser.sng_parser.utils import relation
from ssl import SSL_ERROR_WANT_WRITE

from numpy.core.fromnumeric import squeeze
from numpy.testing._private.utils import assert_equal

#from torch._C import LongStorageBase, LongTensor
...
class abstract_scene(Dataset):
    def __init__(self, config, split=None, transform=None):
        ...
        num_objs = self.cfg.input_vocab_size
        num_preds = 528
        embedding_dim = self.cfg.n_embed
        ...
        self.embedding.weight.data.copy_(self.lang_vocab.vectors)
        self.pred_embeddings = nn.Embedding(num_preds, embedding_dim*2)
    ...
    def __getitem__(self, idx):
        ...
        #print(shuffled_sents)
        word_inds, word_lens = [], []
        triple,obj_ls,obj_inds = [],[],[]
        for k in range(len(shuffled_sents)):
            ...
            for i,r in enumerate(rels):
                ...
                sub_ind = [cur_tokens.index(m)+6*k for m in sub.split(' ') if m in cur_tokens[:5]]  
                obj_ind = [cur_tokens.index(m)+6*k for m in obj.split(' ') if m in cur_tokens[:5]]
        ...
        if triple == []:
            ...
            assert index.shape[0]<=12       
            index = np.vstack((index,np.zeros((12-ind_lens,2))))
    ...
    def embed(self, inds_ls):
        inds_ls = torch.tensor(inds_ls, dtype=torch.long)
        if len(inds_ls)>2:
           # print(inds_ls)
            inds_ls = inds_ls[:2]
            
        if len(inds_ls)==2:    
            s_vec = self.embedding(inds_ls)      
            s_vecs = torch.cat((s_vec[0],s_vec[1]),0)
            s_vecs = s_vecs.unsqueeze(0)
        else:           
            s_vec = self.embedding(inds_ls)
            pad = torch.zeros(1,300)
            #pad = torch.ones(300)
            #pad = s_vec
            s_vecs = torch.cat((s_vec,pad),1)
        return s_vecs
    ...
    def build_lang_vocab(self, scenedb):
        ...
        if osp.exists(lang_vocab_path):
            ...
            for i in range(len(scenedb)):
                ...
                for j, sents in enumerate(group_sents):
                    for k, sent in enumerate(sents):
                       # print(sents)
                        ...
                        if relations_data is not None:
                            for i, rels in enumerate(relations_data):
                                lang_vocab.addRelation(rels)
                lang_vocab.save(lang_vocab_path)
        lang_vocab.get_glovec()
        return lang_vocab

    # def build_lang_rel(self, scenedb):
    #     n_rels = 1
    #     rel2index = {}
    #     lang_rel_path = osp.join(self.cache_dir, 'abstract_lang_rel.json')
    #     if osp.exists(lang_rel_path):
    #         with open(lang_rel_path, 'r') as fp:
    #             rel_info = json.loads(fp.read())
    #         rel2index = rel_info['rel2index']    
    #     else:
    #         for i in range(len(scenedb)):
    #             group_sents = deepcopy(scenedb[i]['scene_sentences'])
    #             for j, sents in enumerate(group_sents):
    ...
    #                         for i, rels in enumerate(relations_data):
    #                             rel = rels[1]
    #                             if rel not in rel2index:
    #                                 rel2index[rel] = n_rels
    #                                 n_rels += 1
    #                             else:
    #                                 pass
    #             rel_info = {}
    #             rel_info['rel2index'] = rel2index
    #             with open(lang_rel_path, 'w') as fp:
    #                 json.dump(rel_info, fp, indent=4, sort_keys=True)

    #     return rel_info

    def obj2index(self, obj):
        ls_inds = []
        #obj "swing set"
        ls = obj.split()#"swing',' set"
        if len(ls)==1:
            if obj in self.lang_vocab.word2index:
                ls_inds = [self.lang_vocab.word2index[obj],0]
            else:
                ls_inds = [0,0]
        else:
            for i, obj in enumerate(ls):  
                if obj == "he":
                    obj = "mike"
                if obj not in self.lang_vocab.word2index:
                    obj = "<pad>"
                obj_inds = self.lang_vocab.word2index[obj] 
                ls_inds.append(obj_inds)
        if len(ls_inds)!=2:
            ls_inds = ls_inds[:2]
        return ls_inds
