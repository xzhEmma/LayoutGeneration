...
class SupervisedTrainer(object):
    ...
    def batch_data(self, entry):
        ...
        return input_inds, input_lens, bg_imgs, fg_onehots, fg_hmaps, gt_inds, gt_msks, gt_scene_inds,index, ind_lens
    ...
    def evaluate(self, inf_outs, ref_inds, ref_msks):
        ...
        if self.cfg.where_attn > 0:
            ...
            if 'loss' in inf_outs.keys():
                attn_loss = attn_loss + pos_att_loss + inf_outs['loss']
    ...
    def train_epoch(self, train_db, optimizer, epoch):
        ...
        for cnt, batched in enumerate(train_loader):
            ...
            gt_inds, gt_msks, gt_scene_inds, index, ind_lens = \
            ...
            if self.cfg.teacher_forcing:
                inf_outs = self.net.teacher_forcing(input_inds, input_lens, bg_imgs, fg_onehots, fg_hmaps, index, ind_lens)
            else:
    ...
    def validate_epoch(self, val_db):
        ...
        for G in range(2):
            ...
            for cnt, batched in enumerate(val_loader):
                ...
                self.net.eval()
                with torch.no_grad():
                    _, env = self.net(input_inds, input_lens, -1, 2.0, 0,None, index, ind_lens)
                    ...
                    inf_outs, _ = self.net(input_inds, input_lens, -1, -0.1, 0, gt_inds,index, ind_lens)
    ...
    def sample(self, epoch, test_db, N, random_or_not=False):
        ...
        for i in indices:
            ...
            index_np = entry['index']
            ind_lens = torch.Tensor([entry['ind_lens']]).long().unsqueeze(0)
            # objs_np = entry['objs']
            # objs_n = torch.Tensor([entry['objs_n']]).long().unsqueeze(0)

            input_inds = torch.from_numpy(input_inds_np).long().unsqueeze(0)
            input_lens = torch.from_numpy(input_lens_np).long().unsqueeze(0)
            index = torch.from_numpy(index_np).long().unsqueeze(0)
            # objs = torch.from_numpy(objs_np).long().unsqueeze(0)
            ...
            self.net.eval()
            with torch.no_grad():
                inf_outs, env = self.net(input_inds, input_lens, -1, 2.0, 0, None,index, ind_lens)
